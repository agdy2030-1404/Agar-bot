...
import { fetchMessages, processMessages, fetchUserAds } from '@/redux/messages/messageSlice';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';

const Messages = () => {
  const dispatch = useDispatch();
  const { messages, loading, processing, error, userAds } = useSelector((state) => state.messages);
  ...
  const handleProcess = () => {
    ...
    dispatch(processMessages(adId)).then(() => {
      dispatch(fetchMessages({ status: '', page: 1, limit: 20 }));
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          <div className="bg-gradient-to-r from-purple-600 to-indigo-700 p-6 text-white">
            ...
            <p className="text-purple-200 mt-1">عرض وإدارة جميع رسائل العملاء</p>
          </div>
          
          <div className="p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
              <div className="flex flex-col">
                <h3 className="text-lg font-semibold text-gray-800">الرسائل الواردة</h3>
                <p className="text-gray-500 text-sm">إجمالي الرسائل: {messages.length}</p>
              </div>
