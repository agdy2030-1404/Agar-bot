...
const AdsTable = () => {
  const { ads, loading: adsLoading, error: adsError } = useSelector(
    (state) => state.bot
  );
        ...
        <div className="overflow-x-auto rounded-2xl border border-gray-200">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-6 py-4 text-right font-medium text-gray-600 uppercase tracking-wider">
                  الإعلان
                </th>
                <th scope="col" className="px-6 py-4 text-right font-medium text-gray-600 uppercase tracking-wider">
                  السعر
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {ads.map((ad) => (
                <tr key={ad.adId || ad._id} className="hover:bg-gray-50 transition-colors duration-200">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-sm text-gray-500 font-mono">{ad.adId}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-lg font-semibold text-gray-900">
                    {ad.price ? `${ad.price} ر.س` : 'غير محدد'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-700">
                    {ad.area ? `${ad.area} م²` : 'غير محدد'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-gray-700">
                    {ad.rooms || 'غير محدد'}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex space-x-2 justify-end">
                      <button
                        ...
                        disabled={loadingAd === ad.adId || ad.status !== "active"}
                      ...
                      <button
                        ...
                        disabled={processingMessages === ad.adId || ad.status !== "active"}
                        className={`px-4 py-2 rounded-xl font-medium transition-all duration-300 flex items-center ${
                          processingMessages === ad.adId || ad.status !== "active"
                            ? "bg-gray-200 text-gray-400 cursor-not-allowed"
